...
if(SPIPACK_USE_LIBC11)
    ExternalProject_Add(
        ...
        BUILD_COMMAND ${BOOST_BUILD_DIR}/b2 cxxflags=${BOOST_CXX_FLAGS} linkflags=${BOOST_LINK_FLAGS} variant=release --user-config=${BOOST_BUILD_DIR}/user-config.jam toolset=${BOOST_TOOLSET_NAME}-spipack  --with-filesystem --with-graph --with-system --disable-icu install &> boost-build.txt
        BUILD_IN_SOURCE 1
        INSTALL_COMMAND ""
        )
else(SPIPACK_USE_LIBC11)
        ExternalProject_Add(
          ...
          UPDATE_COMMAND ""
          CONFIGURE_COMMAND mv ${CMAKE_CURRENT_BINARY_DIR}/user-config.jam ${BOOST_BUILD_DIR}/user-config.jam && ${BOOST_BUILD_DIR}/bootstrap.sh --prefix=${Boost_INSTALL_DIR} --without-icu &> boost-configure.txt
          BUILD_COMMAND ${BOOST_BUILD_DIR}/b2 cxxflags=${BOOST_CXX_FLAGS} variant=release --user-config=${BOOST_BUILD_DIR}/user-config.jam toolset=${BOOST_TOOLSET_NAME}-spipack --with-filesystem --with-graph --disable-icu --with-system  install &> boost-build.txt
          BUILD_IN_SOURCE 1
          INSTALL_COMMAND ""
          )
 endif(SPIPACK_USE_LIBC11)
